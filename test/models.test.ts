...
describe("Feature 2: Model Definitions", () => {
  ...
  describe("bootstrap model catalog", () => {
    ...
    it("uses verified image capabilities for concrete bootstrap models", () => {
      const claudeModels = kiroModels.filter((model) => model.id.startsWith("claude-"));
      const luna = kiroModels.find((model) => model.id === "gpt-5-6-luna");
      ...
      expect(luna?.input).toEqual(["text", "image"]);
